var u = (s => typeof require < "u" ? require : typeof Proxy < "u" ? new Proxy(s, { get: (t, e) => (typeof require < "u" ? require : t)[e] }) : s)(function (s) { if (typeof require < "u") return require.apply(this, arguments); throw Error('Dynamic require of "' + s + '" is not supported') }); var f = (s, t) => () => (t || s((t = { exports: {} }).exports, t), t.exports); var y = f((w, d) => { var I = { calcItemTotal: function (s) { let t = s.price || 0, e = s.quantity || 1; return (t * e).toFixed(2) }, calcDeliveryFee: function (s) { if (s.length === 0) return 0; return s[0].deliveryFee; }, calcPaymentMethod: function (s) { if (s.length === 0) return 0; return s.paymentMethod; }, calcDiscountAmount: function (s) { if (s.length === 0) return 0; return s[0].discountAmount; }, calcSubTotal: function (s) { if (s.length === 0) return 0; let t = 0; return s.forEach(e => { t += Number(this.calcItemTotal(e)); }), t.toFixed(2) }, calcTax: function (s) { if (s.length === 0) return 0; let t = 0; return s.forEach(e => { let c = e.price, a = e.quantity, o = e.tax || 0; t += c * a * o / 100; }), t.toFixed(2) }, calcFinalTotal: function (s, i) { if (s.length === 0) return 0; let t = Number(this.calcSubTotal(s)), d = Number(this.calcDeliveryFee(s)), e = Number(this.calcTax(s)), c = (this.calcDiscountAmount(s)); return ((t + e + d) - c).toFixed(2) } }; d.exports = I; }); var p = f((C, g) => { var T = { string: { invoice: "I N V O I C E", refNumber: "Ref no ", date: "Date", billTo: "Bill To", item: "Item",shapeName: "Shape", flavourName: "Flavour", toppingName: "Topping", colorCode: "Color", quantity: "Qty", price: "Price", total: "Total", subTotal: "Subtotal", deliveryFee: "Delivery Fee", paymentMethod: "Payment Method", discountAmount: "Discount"} }; g.exports = T; }); var v = u("fs"), k = u("path"), q = u("pdfmake"), r = d(), S = g(), p = class {
    payload; company; invoice; customer; items; currency; path; qr; note; date; config; constructor(t, e = S) { this.payload = t, this.company = t.company, this.customer = t.customer, this.invoice = t.invoice, this.items = t.items, this.qr = t.qr, this.note = t.note, this.currency = this.invoice.currency || "$", this.path = k.resolve(this.invoice.path) || "./invoice.pdf", this.date = new Date().toLocaleDateString("en-US", { year: "numeric", month: "numeric", day: "numeric" }), this.config = e; } async create() { let t = new q(this.fonts()), e = { pageSize: "A4", orientation: "portrait", pageMargins: [40, 15, 40, 40], info: this.docMeta(), content: this.content(), defaultStyle: this.docStyle(), styles: this.docTypo() }; return new Promise((c, a) => { let o = t.createPdfKitDocument(e), l = v.createWriteStream(this.path); o.pipe(l), o.on("end", () => c(this.path)), o.on("error", i => a(i)), o.end(); }) } fonts() { let t = { Helvetica: { normal: "Helvetica", bold: "Helvetica-Bold", italics: "Helvetica-Oblique", bolditalics: "Helvetica-BoldOblique" }, Times: { normal: "Times-Roman", bold: "Times-Bold", italics: "Times-Italic", bolditalics: "Times-BoldItalic" }, Courier: { normal: "Courier", bold: "Courier-Bold", italics: "Courier-Oblique", bolditalics: "Courier-BoldOblique" } }, e = this.config?.font || null; return e && Object.keys(e).length > 0 && Object.assign(t, e), t } docMeta() { return { title: "Invoice - #" + this.invoice.number, author: this.company.name, subject: "Invoice - " + this.customer.name, keywords: "invoice" } } docStyle() { let t = { font: "Helvetica", fontSize: this.config?.style?.fontSize || 10, lineHeight: 1.8, bold: !1, color: "#000000", columnGap: 30 }, e = this.config.style || null; return e && Object.keys(e).length > 0 && (t = { ...t, ...e }), t } docTypo() { return { h1: { fontSize: 18, bold: !0 }, h2: { fontSize: 16, bold: !0 }, h3: { fontSize: 14, bold: !0 }, text: { fontSize: this.config?.style?.fontSize || 10, bold: !1 }, textBold: { fontSize: this.config?.style?.fontSize || 10, bold: !0 } } } content() {
      let t = [],
        ...
        e = { columns: [{ width: "30%", stack: [], style: "text" },{ width: "35%", stack: [], style: "text" }, { width: "35%", stack: [], style: "text" }] }; if (this.company.logo) { if (!this.company.logo.startsWith("<svg")) throw new Error("Only SVG logo are supported."); e.columns[1].stack.unshift({ svg: this.company.logo, margin: [0, 65, 0, 0] }), e.columns[0].stack.push({ text: this.company.name, style: "h3", margin: [0, 25, 0, 0] }); } else e.columns[0].stack.unshift({ text: this.company.name, style: "h2" }); this.company.phone && e.columns[0].stack.push({ text: this.company.phone, style: "text" }),this.company.email && e.columns[0].stack.push({ text: this.company.email, style: "text" }), this.company.address && e.columns[0].stack.push({ columns: [{ margin: [0, 0, 0, 20], stack: [{ text: this.company.address }], style: "text" }] }), this.invoice.label ? e.columns[0].stack.unshift({ text: this.invoice.label, style: "h1" }) : e.columns[2].stack.unshift({ text: this.config.string.invoice || "I N V O I C E", style: "h1", margin: [0, 25, 0, 0]}); let c = this.config.string.refNumber || "Ref no :"; e.columns[2].stack.push({ text: c + ": #" + (this.invoice.number || 1), style: "textBold" }); let a = this.config.string.date; e.columns[2].stack.push({ text: a + ": " + (this.invoice.date || this.date), style: "textBold" }); e.columns[2].stack.push({ columns: [{ width: 300, margin: [0, 15, 0, 0], stack: [{ text: this.config.string.billTo, style: "h2" }], style: "text" }] }); this.customer.name && e.columns[2].stack.push({ text: this.customer.name, style: "textBold" }), this.customer.company && e.columns[2].stack.push({ text: this.customer.company, style: "text" }), this.customer.phone && e.columns[2].stack.push({ text: this.customer.phone, style: "text" }); t.push(e); let m = {
        margin: [0, 10, 0, 0],fontSize:7, lineHeight: 1.1,borderColor:'#111111', table: {
            widths: [ 40, 35, 40, 40, 30, 20, 25, 30], headerRows: 1, lineHeight: 1.1,borderColor:'#000000',body: [[
              { text: `${this.config.string.item}`, style: "textBold", fontSize:7 },
              { text: `${this.config.string.shapeName}`, style: "textBold", fontSize:7 },
              { text: `${this.config.string.flavourName}`, style: "textBold", fontSize:7 },
              { text: `${this.config.string.toppingName}`, style: "textBold", fontSize:7 },
              { text: `${this.config.string.colorCode}`, style: "textBold", fontSize:7 },
              { text: `${this.config.string.quantity}`, style: "textBold", fontSize:7 },
              { text: `${this.config.string.price}`, style: "textBold", fontSize:7 },
              { text: `${this.config.string.total}`, style: "textBold", fontSize:7 },
            ]]},
            layout: {
            ...
            ]);}),b.columns[0].stack.push(m);let x={margin:[-20,0,100,0],columns:[
